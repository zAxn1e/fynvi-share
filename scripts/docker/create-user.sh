...
if ! getent group fynvi-share-group > /dev/null 2>&1 && ! getent group pingvin-share-group > /dev/null 2>&1; then
    addgroup -g "$PGID" fynvi-share-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u fynvi-share > /dev/null 2>&1 && ! id -u pingvin-share > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G fynvi-share-group fynvi-share > /dev/null 2>&1
    else
