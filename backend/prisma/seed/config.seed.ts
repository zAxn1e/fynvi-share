...
export const configVariables = {
  ...
  email: {
    ...
    resetPasswordSubject: {
      type: "string",
      defaultValue: "Fynvi Share password reset",
    },
    ...
    inviteSubject: {
      type: "string",
      defaultValue: "Fynvi Share invite",
    },
    inviteMessage: {
      type: "text",
      defaultValue:
        'Hey!\n\nYou were invited to Fynvi Share. Click this link to accept the invite: {url}\n\nYou can use the email "{email}" and the password "{password}" to sign in.\n\nFynvi Share',
    },
    ...
    verificationSubject: {
      type: "string",
      defaultValue: "Verify your Fynvi Share account",
    },
    verificationMessage: {
      type: "text",
      defaultValue:
        "Hey!\n\nYou just signed up for Fynvi Share. Click this link to verify your account: {url}\n\nThe link expires in 24 hours.\n\nFynvi Share",
    },
  },
  smtp: {
    enabled: {
...
async function migrateConfigVariables() {
  ...
  for (const existingConfigVariable of existingConfigVariables) {
    ...
    if (!configVariable) {
      ...
      let val = existingConfigVariable.value;
      if (
        existingConfigVariable.category === "general" &&
        existingConfigVariable.name === "appName" &&
        (!val || val.includes("Pingvin") || val.includes("Noctra"))
      ) {
        val = "Fynvi Share";
      }
